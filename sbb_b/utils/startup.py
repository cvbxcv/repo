...
LOGS = logging.getLogger("اعداد ثري ثون")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@CCYFC"))
        await sbb_b(JoinChannelRequest("@RR7PP"))
        await sbb_b(JoinChannelRequest("@CCYFC_help"))
...
STRINGS = {
    1: """🎇 **- شكراً لتنصيبك سورس ثري ثون **
•• من الاسفل بعض الخيارات التي ستساعدك في ثري ثون.""",
    2: """🎉** حول ثري ثون**
🧿 ثري ثون هو يوزربوت في مكتبة التيليثون تم صنعه بأستخدام البايثون. يحتوي على اكثر من 100 أمر تساعدك في التليجرام و ثري ثون هو افضل سورس من ناحية الامان.
❣ قناة السورس **@CCYFC**""",
    3: """**💡• قنوات السورس •**

قناة السورس: @CCYFC
قناة المساعدة: @CCYFC_HELP
مجموعة المساعدة: @CCYFC_SUPPORT""",
    4: f"""• `لمعرفة جميع اوامر السورس ارسل`
  - `.اوامري`
  - `.الاوامر`""",
    5: """• **لأي مساعدة ثانية **
  - أنضم في مجموعة المساعدة **@CCYFC_support**.
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await sbb_b.upload_file(file="razan/pic/rer.jpg")
        _, groupid = await create_supergroup(
            "كروب بوت ثري ثون", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
