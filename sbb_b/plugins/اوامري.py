...
ROE = """** اهلا بك عزيزي المستخدم في قائمة اوامر ثري ثون
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == sbb_b.uid:
            buttons = [
                [Button.inline("معلومات ثري ثون", data="AOMRDB")],
                [
                ...
                [
                    ...
                    Button.inline(" الفارات", data="VARCCYFC"),
                ],
            ]
...
@sbb_b.tgbot.on(CallbackQuery(data=re.compile(rb"VARCCYFC")))
...
async def varssett(event):
    await event.edit(
        ...
        buttons=[
            ...
            [Button.inline("رجوع", data="VARCCYFC")],
            [Button.inline("القائمة الرئيسية", data="MAIN")],
        ],
    )
...
اوامر فارات سورس ثري ثون @CCYFC""",
        buttons=[
...
async def varssett(event):
    await event.edit(
        """اهلا بك في قائمة اوامر سورس ثري ثون هذه بعض المعلومات عن ثري ثون:

- سورس ثري ثون يمتلك اكثر من 100 أمر 
- ثري ثون هو افضل سورس عربي يتميز بالحماية

قناة الكلايش:  @CCYFC
قناة الملاحظات: @CCYFC
قناة السورس: @CCYFC
...
* يمكنك الحصول على  كليشة جاهزة من هذه القناة @CCYFC
ملاحظة : يمكنك استخدام الاوامر في اي دردشة او محادثة
اوامر فارات سورس ثري ثون @CCYFC""",
        buttons=[
            [Button.inline("رجوع", data="pmvars")],
